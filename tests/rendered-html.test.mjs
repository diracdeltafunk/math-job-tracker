...
test("keeps personalization deployment-only and the application-link field removed", async () => {
  ...
  assert.match(triageRoute, /candidate profile's automatic exclusions/);
  assert.match(triageRoute, /Do not infer desirability from the title alone/);
  assert.doesNotMatch(triageRoute, /conservative fit decision|For R1 research jobs|undesireable/);
  assert.match(viteConfig, /TRIAGE_PROFILE/);
  assert.match(workflow, /secrets\.TRIAGE_PROFILE/);
  assert.match(workflow, /secrets:\s*\|\s*TRIAGE_PROFILE/);
  assert.match(guide, /Worker\s+secret[^]*Wrangler summarizes plain binding values/i);
  ...
  assert.match(guide, /generic triage\s+prompt—not the candidate profile—\s*handles how titles map/i);
  assert.doesNotMatch(guide, /What timezone should be used when a deadline date has no stated time/i);
